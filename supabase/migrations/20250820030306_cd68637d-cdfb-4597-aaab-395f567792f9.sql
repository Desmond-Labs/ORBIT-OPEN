-- Fix calculate_hybrid_pricing to explicitly cast NULL to correct type
...
BEGIN
    ...
    IF paid_images > 0 THEN
        -- Always pass 3 parameters with explicit type casting
        IF billing_period_start_param IS NULL THEN
            SELECT calculate_tier_pricing(user_id_param, paid_images, NULL::timestamp with time zone) INTO paid_pricing;
        ELSE
